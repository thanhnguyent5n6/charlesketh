	...
	<div class="col-md-12">
        <div class="portlet">
            <div class="portlet-body">
                <form role="form" method="GET" id="form-search" class="form-inline text-right" action="{{ route('admin.pos.index') }}" >
                    ...
                    <div class="form-group">
                        <a href="{{ route('admin.pos.index',['type'=>$type]) }}" class="btn default"> <i class="fa fa-refresh"></i> Đặt lại</a>
		...
		<div class="portlet box green">
			<div class="portlet-title">
                ...
                <div class="actions">
                    <a href="{{ route('admin.pos.create',['type'=>$type]) }}" class="btn btn-sm btn-default"> Thêm mới </a>
					...
					<table class="table table-bordered table-condensed">
						...
						<tbody>
                            ...
                            <tr id="record-{{ $item->id }}">
                                ...
                                @if ( Auth::user()->hasRole('admin') || Auth::user()->groups()->first()->id == 7  )
                                <td align="center"><a href="{{ route('admin.pos.edit',['id'=>$item->id, 'type'=>$type]) }}"> {{ $item->name.' - '.$item->phone }} </a></td>
                                @else
                                <td align="center"><a href="{{ route('admin.pos.show',['id'=>$item->id, 'type'=>$type]) }}"> {{ $item->name.' - '.$item->phone }} </a></td>
                                @endif
                                ...
                                <td align="center">
                                    ...
                                    <a href="#" style="margin-top: 5px;" data-target="#cancel-modal" data-toggle="modal" data-url="{{ route('admin.pos.update',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm btn-cancel blue" title="Thanh toán"> Thanh toán {!! $item->received_amount ? get_currency_vn($item->received_amount,'') : '' !!}</a>

                                    @if($item->deposit_amount)
                                    <a href="#" data-target="#cancel-modal" data-toggle="modal" data-url="{{ route('admin.pos.update',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm btn-cancel blue" title="Đã thu cọc"> Đã thanh toán {!! get_currency_vn($item->deposit_amount,'') !!}</a>
                                    @endif

                                    @if($item->installation_fees)
                                    <a href="#" data-target="#cancel-modal" data-toggle="modal" data-url="{{ route('admin.pos.update',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm btn-cancel blue" title="Đã thu cọc"> Phí lắp đặt {!! get_currency_vn($item->installation_fees,'') !!}</a>
                                    @endif
                                ...
                                <td align="center">
                                    ...
                                    @if( Auth::user()->groups()->first()->id == 7 )
                                    <a href="javascript:;" class="btn btn-sm blue btn-print" title="Phiếu xuất kho giá kê" data-ajax="id={{ $item->id }}|loai=4"> Phiếu XK Kê <i class="fa fa-print"></i></a>
                                    @endif
                                    @if ( Auth::user()->id == 21 )
                                    <a href="javascript:;" class="btn btn-sm blue btn-print" title="Phiếu xuất kho ko giá" data-ajax="id={{ $item->id }}|loai=5"> Phiếu XKKG <i class="fa fa-print"></i></a>
                                    @endif
                                    <br/>
                                    <a href="{{ route('admin.pos.show',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
                                    @else
                                    <a href="{{ route('admin.pos.show',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
                                    @endif
                                </td>
...
<script type="text/javascript">
    $(document).ready(function(){
        $("a.btn-print").on('click', function(e){
            ...
            $.ajax({
                type: 'GET',
                url : Laravel.baseUrl+'/admin/pos/print',
                data: dataAjax,
                dataType: 'json'
            }).done(function(obj){
