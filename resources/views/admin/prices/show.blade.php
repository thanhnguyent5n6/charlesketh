...
<div class="row" id="qh-app">
    ...
    <div class="col-lg-9 col-xs-12">
        ...
        <qh-view :apply_discount="apply_discount" :coupon_amount="coupon_amount" :enhancement="enhancement" :shipping="shipping"></qh-view>
        <div class="portlet box green">
            <div class="portlet-title">
            ...
            <div class="portlet-body">
                ...
                <div class="form-group row">
                    <div class="col-sm-6">
                        ...
                        <div>
                            <select class="selectpicker show-tick show-menu-arrow form-control province validate[required]" name="data[province_id]" data-group="order" data-live-search="true" title="-- Chọn Tỉnh / Thành phố --">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div>
                            <select class="selectpicker show-tick show-menu-arrow form-control district validate[required]" name="data[district_id]" data-group="order" data-live-search="true" title="-- Chọn Quận / Huyện --">
                ...
                <div class="invoice" style=" @php if($item->invoice['company'] == ''){ echo 'display: none;'; } @endphp margin-top: 15px;">
                    ...
                    <div class="form-group">
                        <label class="control-label">Thông Tin Xuất ( Giá xuất / ngày xuất )</label>
                        <div>
                            <input type="text" name="data[invoice][note]" class="form-control" value="">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="col-lg-3 col-xs-12">
        <div class="portlet box green">
            ...
            <div class="portlet-body">
                <div class="form-group bg-red" style="padding: 10px;">
                    <label class="mt-checkbox font-white" style="margin-bottom: 0px;">Hiện giảm giá trên PXK
                    <input type="checkbox" name="status[]" value="apply_discount" v-model="apply_discount" > <span></span> </label>
                </div>
                @if( $type == 'shop' )
                <div class="form-group">
        ...
        <tbody>
            <tr v-for="(item, key) in products" >
                <td align="center">
                    ...
                    <input type="hidden" :name="'products['+ key +'][price_virtual]'" v-model="item.price_virtual">
                    @{{ item.code }}
                </td>
                <td>
                    @{{ item.title }}
                ...
                <td align="center"> 
                    <input type="text" :name="'products['+ key +'][price_increase]'" placeholder="giá thêm" class="validate[required] form-control text-center" v-model.formatPrice="item.price_increase" @keyup="priceIncrease(item)">
                    +
                    <div> @{{ formatPrice( parseInt(item.price) + parseInt(item.price_increase) ) }} </div> + <div>Giá kê hãng</div><div> @{{ formatPrice(item.price_virtual) }} </div>
                </td>
                <td align="center"><div> @{{ formatPrice(item.price_second) }} </div><input type="text" :name="'products['+ key +'][price_second]'" class="form-control text-center validate[required]" v-model.formatPrice="item.price_second"> </td>
                ...
                <td align="center"><div>@{{ formatPrice(subtotal[key]) }} </div> - <div> @{{ formatPrice(item.price_discount) }} </div><input type="text" :name="'products['+ key +'][price_discount]'" class="form-control text-center" placeholder="Giảm giá" v-model.formatPrice="item.price_discount"><span>giảm giá sp</span></td>
...
<script type="text/x-template" id="select2-data-view">
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">Thông tin trên phiếu xuất kho</div>
        </div>
        <div class="portlet-body">
            <div class="table-responsive">
                <table class="table table-bordered table-condensed">
                    ...
                    <tbody>
                        <tr v-for="(item, key) in products" >
                            ...
                            <td align="center">
                                @{{ apply_discount ? formatPrice(item.price_discount) : 0 }}
                            </td>
                            <td align="center">
                                <div v-if=" item.price_virtual == 0 && ((parseInt(item.price)+parseInt(item.price_increase)) == item.price_second) ">@{{ formatPrice(parseInt(item.price)+parseInt(item.price_increase)) }}</div>
                                <div v-else-if=" item.price_virtual == 0 && (item.price_second > (parseInt(item.price)+parseInt(item.price_increase))) ">@{{ formatPrice(item.price_second) }}</div>
                                <div v-else-if=" item.price_virtual > 0 && ( (parseInt(item.price)+parseInt(item.price_increase)+parseInt(item.price_virtual)) > item.price_second) ">@{{ formatPrice(parseInt(item.price)+parseInt(item.price_increase)+parseInt(item.price_virtual)) }}</div>
                                <div v-else-if=" item.price_virtual > 0 && ( (parseInt(item.price)+parseInt(item.price_increase)+parseInt(item.price_virtual)) < item.price_second) ">@{{ formatPrice(item.price_second) }}</div>
                                <div v-else>@{{ formatPrice(item.price) }}</div>
                            </td>
                            <td align="center">
                                @{{ formatPrice(subtotal[key]) }}
                            </td>
                        </tr>
                        <tr v-if="coupon_amount">
                            ...
                            <td>
                                <span class="font-red-mint font-md bold">@{{ formatPrice(coupon_amount) }}</span>
                            </td>
                        </tr>
                        <tr v-if="shipping">
                            <td align="right" colspan="6">
                            ...
                            <td>
                                <span class="font-red-mint font-md bold">@{{ formatPrice(shipping) }}</span>
                            </td>
                        </tr>
                        <tr>
                            <td align="right" colspan="6">
                            ...
                            <td>
                                <span class="font-red-mint font-md bold">@{{ formatPrice(total) }}</span>
                            </td>
                        </tr>
                        <tr v-if="totalVirtual">
                            <td align="right" colspan="6">
                                <span class="pull-right text-uppercase bold text-center">
                                    Cọc lần 1:
                                </span>
                            ...
                            <td>
                                <span class="font-red-mint font-md bold">@{{ formatPrice(totalVirtual) }}</span>
                            </td>
                        </tr>
                        <tr>
                            <td align="right" colspan="6">
                            ...
                            <td>
                                <span class="font-red-mint font-md bold">@{{ formatPrice(total - totalVirtual) }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<script type="text/javascript">
    ...
    new Vue({
        ...
        data: function () {
            ...
            @endif
            return {
                apply_discount: {{ strpos($item->status,'apply_discount') !== false ? 'true' : 'false' }},
        ...
        computed: {
            total() {
                return this.products.reduce((total, item) => {
                    return total + (item.qty * ((parseInt(item.price)+parseInt(item.price_increase))-parseInt(item.price_discount)));
        ...
        components: {
            'qh-products': {
                ...
                computed: {
                    subtotal() {
                        return this.products.map((item) => {
                            return Number( item.qty * (parseInt(item.price)+parseInt(item.price_increase))-parseInt(item.price_discount) )
                        });
                    },
                    total() {
                        return this.products.reduce((total, item) => {
                ...
                methods: {
                    ...
                    priceIncrease(item) {
                        item.price_second = parseInt(item.price) + parseInt(item.price_increase)
                    }
                    
                }
            },
            'qh-view': {
                template: '#select2-data-view',
                props: ['apply_discount','coupon_amount','enhancement','shipping'],
                data: function () {
                    return {
                        products: this.$parent.products
                    };
                ...
                computed: {
                    subtotal() {
                        return this.products.map((item) => {
                            if( item.price_virtual == 0 && ((parseInt(item.price)+parseInt(item.price_increase)) == item.price_second) ){
                                $price = (parseInt(item.price)+parseInt(item.price_increase));
                                $tiencoc = 0;
                            }else if( item.price_virtual == 0 && (item.price_second > (parseInt(item.price)+parseInt(item.price_increase))) ){
                                $price = item.price_second;
                                $tiencoc = 0;
                            }else if( item.price_virtual > 0 && ( (parseInt(item.price)+parseInt(item.price_increase)+parseInt(item.price_virtual)) > item.price_second) ) {
                                $price = (parseInt(item.price)+parseInt(item.price_increase)+parseInt(item.price_virtual));
                                $tiencoc = parseInt($price) - parseInt(item.price_second);
                            }else if( item.price_virtual > 0 && ( (parseInt(item.price)+parseInt(item.price_increase)+parseInt(item.price_virtual)) < item.price_second) ){
                                $price = item.price_second;
                                $tiencoc = 0;
                            }else {
                            ...
                            if( this.apply_discount ) $price -= item.price_discount
                ...
                for (var i = 0; i < select2data.length; i++) {
                    ...
                    if(!flag){
                        ...
                        this.products.push({
                            ...
                            "qty": 1,
                            "price_virtual": select2data[i].vtp_price,
                            "price": price,
                            "price_second": price,
                            "price_discount": 0,
                            "price_increase" : 0
