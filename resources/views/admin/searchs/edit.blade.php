...
@section('breadcrumb')
<li>
    <a href="{{ route('admin.search.index', ['type'=>$type]) }}"> {{ $pageTitle }} </a>
...
<div class="row">
    ...
    <!-- BEGIN FORM-->
    <form role="form" method="POST" action="{{ route('admin.search.update',['id'=>$item->id,'type'=>$type]) }}" enctype="multipart/form-data" >
        {{ csrf_field() }}
        {{ method_field('put') }}
