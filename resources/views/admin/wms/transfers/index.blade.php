	...
	<div class="col-md-12">
		<div class="portlet box green">
			<div class="portlet-title">
                ...
                <div class="actions">
                    <a href="{{ route('admin.wms_transfer.create',['type'=>$type]) }}" class="btn btn-sm btn-default"> Thêm mới </a>
                    {{--
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu pull-right">
                            ...
                            <li>
                                <a href="javascript:;" class="btn-action" data-type="{{ $key }}" data-ajax="act=update_status|table=wms_transfers|col=status|val={{ $key }}"> {{ $act }} </a>
                            </li>
                            @endforeach
                            <li>
                                <a href="javascript:;" class="btn-action" data-type="delete" data-ajax="act=delete_record|table=wms_transfers"> Xóa dữ liệu </a>
                            </li>
                        </ul>
                    </div>
                    --}}
					...
					<table class="table table-bordered table-condensed">
						<thead>
							<tr>
                                ...
                                <th width="7%"> Kho từ </th>
                                <th width="7%"> Kho đến </th>
						...
						<tbody>
                            ...
                            <tr id="record-{{ $item->id }}">
                                ...
                                <td align="center"> <input type="text" name="priority" class="form-control input-mini input-priority" value="{{ $item->priority }}" data-ajax="act=update_priority|table=wms_transfers|id={{ $item->id }}|col=priority"> </td>
                                <td align="center">{{ $item->username }}</td>
                                <td align="center">{{ $item->code }}</td>
                                <td align="center">{{ $item->store_from }}</td>
                                <td align="center">{{ $item->store_to }}</td>
                                <td align="center">{{ $item->transfer_qty }}</td>
                                <td align="center">{!! get_currency_vn($item->transfer_price,'') !!}</td>
                                ...
                                <td align="center">
                                    <a href="{{ route('admin.wms_transfer.edit',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
                                    @if($item->status == 'publish')
                                    <a href="#" data-target="#cancel-modal" data-toggle="modal" data-url="{{ route('admin.wms_transfer.delete',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm btn-cancel red" title="Hủy phiếu"> <i class="fa fa-ban"></i> </a>
                                    @endif
                                </td>
                            </tr>
                            @empty
