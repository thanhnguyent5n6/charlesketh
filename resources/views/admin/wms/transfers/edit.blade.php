...
<div class="row">
    ...
    <!-- BEGIN FORM-->
    <form role="form" method="POST" action="{{ route('admin.wms_transfer.update',['id'=>$item->id,'type'=>$type]) }}" class="form-validation">
        {{ csrf_field() }}
        {{ method_field('put') }}
                        ...
                        <table class="table table-bordered table-condensed">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td align="center">{{ ($product->unit === 1 ? 'Bộ/Cái' : ($product->unit === 2 ? 'Dàn nóng' : 'Dàn lạnh')) }}</td>
                                ...
                                <tr>
                                    <td align="right" colspan="30">
                                        <span class="pull-left text-uppercase">
                                            Số lượng: <span class="font-red-mint font-md bold">{!! get_currency_vn($item->export_qty,'') !!}</span>
                                        </span>
                                        <span class="pull-right text-uppercase">
                                            Tổng: <span class="font-red-mint font-md bold">{!! get_currency_vn($item->export_price,'') !!}</span>
                                        </span>
                                    </td>
                                </tr>
                            </tbody>
            ...
            <div class="portlet box green">
                ...
                <div class="portlet-body">
                    <div class="form-group">
                        ...
                        <div>
                            <select name="data[store_from]" class="selectpicker show-tick show-menu-arrow form-control" disabled="disabled">
                                @forelse($warehouses as $warehouse)
                                <option value="{{ $warehouse->code }}" {{ $warehouse->code == $item->store_from ? 'selected' : '' }} > {{ $warehouse->name }} </option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <select name="data[store_to]" class="selectpicker show-tick show-menu-arrow form-control" disabled="disabled">
                                @forelse($warehouses as $warehouse)
                                <option value="{{ $warehouse->code }}" {{ $warehouse->code == $item->store_to ? 'selected' : '' }} > {{ $warehouse->name }} </option>
                                @empty
                                @endforelse
                            </select>
                        </div>
